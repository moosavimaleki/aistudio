...
function renderProfiles(profiles) {
  $("#profiles").innerHTML = profiles.length ? profiles.map(profile => `
    <tr>
      <td><code>${escapeHtml(profile.browser_id)}</code><br><small>auth ${escapeHtml(profile.auth_user)}</small></td>
      <td>${pill(profile.session_state)}</td>
      <td>${pill(profile.connected ? "READY" : "DISCONNECTED")}</td>
      <td>${number(profile.cookie_count)} <small>(${number(profile.auth_cookie_count)} auth)</small></td>
      <td>${number(profile.cookie_revision)} ${profile.cookie_source_current ? "" : pill("STALE")}</td>
      <td>${dateTime(profile.cookie_expires_at)}</td>
      <td>${profile.heartbeat_age_seconds == null ? "—" : `${profile.heartbeat_age_seconds}s`}</td>
      <td title="${escapeHtml(profile.warm_error)}">${profile.warm_error ? pill("ERROR") : pill(profile.ready ? "READY" : "WARMING")}</td>
    </tr>`).join("") : `<tr><td colspan="8" class="empty">Profileای پیدا نشد</td></tr>`;
}
...
function renderEvents(events) {
  $("#events").innerHTML = events.length ? events.map(event => {
    const details = Object.entries(event)
      .filter(([key]) => !["timestamp", "category", "event"].includes(key))
