...
const probeScript = `async function(input) {
  let violation;
  const captureViolation = (event) => {
    violation = {
      blockedURI: event.blockedURI,
      violatedDirective: event.violatedDirective,
    };
  };
  document.addEventListener("securitypolicyviolation", captureViolation, {once: true});
  try {
    ...
    document.removeEventListener("securitypolicyviolation", captureViolation);
  }
}`
